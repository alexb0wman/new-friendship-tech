...
**Local demo works without credentials or a wallet. Production integration code is implemented, with checkout disabled by default until configured and exercised. ENSv2 is Sepolia-only; existing ENS names, World proofs and payments have production paths. Read [the live launch spec](docs/LIVE-LAUNCH-SPEC.md).**
...
- ENS normalization, fresh mainnet or Sepolia resolution, wallet association, lookup, simulated resolver permission check, wallet-signed description updates and post-transaction re-read. Real RPC/wallet exercise remains unverified.
- ENSv2 trips: an expiring, non-transferable subname per member per city (`maya.tokyo.<parent>.eth`, expiry = departure), minted only after a World ID proof of human, with records proven by re-read. Tables: data-only subnames under `tables.<city>` whose attendee record (names only) is written by a concierge wallet that holds setter roles for two text keys and nothing else. Split the bill from names, verified on chain. One concierge agent with ENSIP-26 records and a read-only MCP endpoint; every action that puts a member in a room is approved through production World ID human-in-the-loop proofs. Demo and tests run on simulated adapters; see `docs/ENS-WORLD-DESIGN.md`.
...
1. **Live external evidence:** membership checkout uses server-owned TokenFlight quotes, Base USDC source verification and native 0G settlement verification. Configure a treasury and RPCs, then exercise a real purchase. No live purchase is claimed by this branch.
2. **ENSv2 availability and World configuration:** ENSv2 mainnet is unavailable. Bootstrap and execute the live Sepolia namespace, while existing-name identity can use Ethereum mainnet. Configure the production World app to exercise IDKit trip proofs and action-bound concierge approvals. Sandbox OIDC is no longer required.
3. **Privy, KMS and GCP:** code is present; credentials, allowlisted domains, permissions and live integration testing are still required. Deployment and live credentials are outside this checked-in implementation.
