...
export async function getInvoice(actorId: string, id: string) {
  const [row] = await (
    ...
    .select()
    .from(s.invoices)
    .where(and(eq(s.invoices.id, id), eq(s.invoices.userId, actorId)));
  invariant(row, "NOT_FOUND", "Invoice not found.", 404);
...
export async function createInvoice(actorId: string, body: z.infer<typeof invoiceSchema>) {
  ...
  const [existing] = await db
    .select()
    .from(s.invoices)
    .where(and(eq(s.invoices.userId, actorId), eq(s.invoices.idempotencyKey, body.idempotencyKey)));
  ...
  const [wallet] = await db
    .select()
    .from(s.walletLinks)
    .where(and(eq(s.walletLinks.userId, actorId), eq(s.walletLinks.address, body.sourceWallet)));
  ...
  return applyWrite(actorId, "invoice.create", body.idempotencyKey, async (tx) => {
    ...
    const [again] = await tx
      ...
      .from(s.invoices)
      .where(
        and(eq(s.invoices.userId, actorId), eq(s.invoices.idempotencyKey, body.idempotencyKey)),
      );
    if (again) return invoiceDTO(again);
    const [pending] = await tx
      .select()
      .from(s.invoices)
      .where(and(eq(s.invoices.userId, actorId), inFlight()))
      .limit(1);
    invariant(
      !pending,
      "PENDING_INVOICE",
      "Finish or wait for your existing invoice before starting another.",
      409,
    );
    const [row] = await tx
      .insert(s.invoices)
...
function inFlight() {
  return or(
    and(eq(s.invoices.status, "quoted"), gt(s.invoices.quoteExpiresAt, new Date())),
    eq(s.invoices.status, "submitted"),
    eq(s.invoices.status, "confirming"),
    eq(s.invoices.status, "review_required"),
  );
}
...
export async function recordSubmission(
  ...
  await applyWrite(actorId, "invoice.submit_hint", id, async (tx) => {
    ...
    invariant(
      invoice.status === "quoted" && !invoice.sourceTx,
      "INVOICE_STATE",
      "This invoice already has a submission.",
      409,
    );
...
export async function simulatePayment(actorId: string, id: string) {
  ...
  await applyWrite(actorId, "demo.payment.submit", id, async (tx) => {
    ...
    invariant(
      row.status === "quoted" && row.quoteExpiresAt > new Date(),
      "QUOTE_EXPIRED",
      "This quote expired. Create a new invoice.",
      409,
    );
    await tx
      .update(s.invoices)
      .set({ status: "submitted", sourceTx: demoTransactionHash() })
      .where(eq(s.invoices.id, id));
...
export async function reconcileInvoice(id: string) {
  ...
  invariant(
    ["submitted", "confirming", "review_required"].includes(snapshot.status),
    "INVOICE_STATE",
  ...
  await db.transaction(async (tx) => {
    ...
    await tx
      ...
      .from(s.users)
      .where(eq(s.users.id, snapshot.userId))
      .for("update");
    ...
    invariant(
      ...
      "INVOICE_STATE",
      "Invoice cannot settle.",
      409,
    );
    const [used] = await tx
      .select()
      .from(s.settlements)
      .where(eq(s.settlements.settlementKey, verified.key));
    invariant(
      !used || used.invoiceId === id,
      "PAYMENT_REPLAY",
      "This settlement has already been used.",
      409,
    );
    const [latest] = await tx
      .select()
      .from(s.entitlements)
      .where(eq(s.entitlements.userId, invoice.userId))
    ...
    await tx
      ...
      .where(eq(s.jobs.invoiceId, id));
    await audit(
      tx,
      null,
      invoice.provider === "demo" ? "demo.payment.settled" : "payment.settled",
      id,
    );
  });
}
export async function runWorkerOnce(owner = randomUUID()) {
  ...
  const job = await db.transaction(async (tx) => {
    const [row] = await tx
      ...
      .where(
        and(
          lte(s.jobs.runAfter, now),
          or(
            eq(s.jobs.status, "ready"),
            and(eq(s.jobs.status, "running"), lte(s.jobs.leaseUntil, now)),
          ),
        ),
      )
      .orderBy(asc(s.jobs.runAfter))
      .limit(1)
  ...
  try {
    ...
    await applyWrite(null, "payment.reconcile_retry", job.invoiceId, async (tx) => {
      await tx
        ...
        .where(and(eq(s.jobs.id, job.id), eq(s.jobs.leaseOwner, owner)));
      if (terminal)
        await tx
          .update(s.invoices)
          .set({ status: "review_required", failureCode: code })
          .where(and(eq(s.invoices.id, job.invoiceId), neSettled()));
    });
    return true;
  }
}
...
export async function listInvoices(actorId: string) {
  const rows = await (
    await getDb()
  )
    .select()
    .from(s.invoices)
    .where(eq(s.invoices.userId, actorId))
