...
### 2.9 0G Pay is a flag and a note, not a mount

Spec section 7.5 mounts `OgPayTrigger` for the split when `SPLIT_OGPAY_ENABLED=true`. Bundling that component fails on `main` today: `@0gfoundation/0g-pay-sdk` depends on `@tokenflight/adapter-ethers`, which requires the `ethers` package, and the repository does not install it (the trigger is never mounted anywhere on `main`, so the build never noticed). Adding a dependency for an unverified payment route is the owner's call, not this drop's, so the split pays with a plain USDC transfer from the member's wallet, and the flag renders a note explaining what to add. Mounting 0G is then: `npm install --save-exact ethers@<version>` and drop `OgPayTrigger` into `PaySection` in `src/components/views/table-detail.tsx` with the resolved recipient and amount. The hint path (`POST /gatherings/:id/split/paid`) already accepts whatever hash the provider returns.

### 2.10 Right now keeps its direct path

Spec section 8 turns the Right now composer into "Ask the concierge to post it". The composer does that when the member has an active trip and a linked World ID, and otherwise posts directly as before, so members who have not activated a trip keep the feature they have today.
